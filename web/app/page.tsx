...
export default function DashboardPage() {
  const { signals, loading, error, lastUpdate, refresh } = useSignals();
  ...
  async function vote(symbol: string, side: "bull" | "bear") {
    ...
    try {
      ...
      const vd = await vr.json();
      if (vd.success) setVotes((prev) => ({ ...prev, [symbol]: vd.votes }));
    } catch (e) {
      console.error(e);
    }
  }
  ...
  return (
    <TerminalLayout>
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="text-right">
            ...
            <button
              onClick={refresh}
              disabled={loading}
              className="mt-1 border border-gray-700 text-gray-500 px-3 py-1 text-[10px] hover:border-cyan hover:text-cyan transition-colors disabled:opacity-50"
            >
        ...
        {error && (
          <div className="mb-4 border border-yellow-600 bg-panelBg p-3 text-xs text-yellow-500">
            ⚠ {error}
          </div>
        )}
        ...
        <div className="space-y-3">
          {loading && signals.length === 0 ? (
            <div className="text-gray-500 text-xs animate-pulse py-8 text-center">LOADING SIGNALS FROM BYBIT...</div>
          ) : signals.length === 0 ? (
            <div className="text-gray-500 text-xs py-8 text-center">NO SIGNALS AVAILABLE</div>
          ) : (
            signals.map((sig, i) => {
                    ...
                    {voteData && voteData.total > 0 && (
                      <div className="flex-1 flex items-center gap-2">
                        <div className="flex-1 h-1.5 bg-gray-800 rounded-full overflow-hidden">
                          <div className="h-full bg-neonGreen" style={{ width: `${voteData.bullRatio * 100}%` }} />
                        </div>
